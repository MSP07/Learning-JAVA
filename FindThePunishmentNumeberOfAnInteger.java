...
class Solution {
    // private boolean isPossible(String s,int target,int ind,int sum){
    //     if(ind == s.length())return sum == target;
    //     int num = 0;
    ...
    //     return false;
    // }
    private boolean isPossible(int sqr,int org){
        if(org<0 || sqr<org)return false;
